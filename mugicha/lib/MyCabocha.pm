    ...
    for my $text (@{ $self->get_text }) {
        for my $chunk (@{ $text->get_chunk }) {
            for my $morph (@{ $chunk->get_morph }) {
                for my $id (split /\s+/, $morph->get_argstr) {
                    if ($id =~ m/id=\"([^"]*)\"/gmx) {
                        $arg_of{$1} = $morph;
                    }
                }
...
sub new {
    ...
    # FIXME: new layout does not use 8th field.
    $self->set_argstr(  $labels[7]);
    ...
    # set next and prev morph
    if ($self->get_id > 0) {
        $self->{prev} = \${$self->get_parent}->get_morph_by_id($self->get_id - 1);
        ${$self->prev}->{next} = \$self;
    }
...
sub set_case {
    ...
    for my $case (qw(ga o ni)) {
        if (my $ln = ($self->get_argstr =~ m/$case="([^"]*)"/igmx)[0]) {
            $self->{$case} = $ln;
        } else {
            $self->{$case} = '';
        }
...
sub get_arg_by_case {
    ...
    my $ln = $self->get_case($case);
    if ($ln =~ m/^\d+/gmx) {
        # intra
        return ${$self->get_cab}->get_arg($ln);
    } else {
        # inter
        return $ln;
    }
