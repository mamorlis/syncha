...
#       print STDERR $mor[8], "\n" if ($mor[8]);
        &set_event_tag($self, $mor[8]) if ($mor[8]);
#       $self->EVENT(&ext_ev($mor[7])) if ($mor[7]);
#       $self->EVENT(&ext_id($mor[7])) if ($mor[7]);
#       $self->EVENT(&ext_arg($mor[7])) if ($mor[7]);
#     } elsif (scalar(@_) > 1) {
#       $self->WF($_[0]);
#       $self->READ($_[1]);
#       $self->BF($_[2]);
...
#     print STDERR "\t", 'copy: ', $copy->STRING, "\n";
#     for my $d (@dtr) {
#       print STDERR "\t\tdtr: ", $d->STRING, "\n";
#     }
...
#     print STDERR $in, "\n";
    my @cab = (); my @mor = (); my $b;
    my @line = split '\n', $in;
    if (@line != 0 and $line[-1] eq 'EOS') {
        pop @line; # delete EOS
    }

    # EOS\nEOS のときには CaboCha オブジェクトを返そうとしない
    return '' unless (@line);
...
sub ext_event_info {
    my @b = @_; my $b_num = @b;
    for (my $bid=0;$bid<$b_num;$bid++) {
        my $b = $b[$bid]; my @m = @{$b->Mor};
        my $h = $m[$b->head];
        $b[$bid]->ID($h->ID) if ($h->ID); $b->EVENT($h->EVENT) if ($h->EVENT);
        $b[$bid]->GA($h->GA) if ($h->GA); $b->WO($h->WO) if ($h->WO);
        $b[$bid]->NI($h->NI) if ($h->NI);
    }
    return @b;
}
